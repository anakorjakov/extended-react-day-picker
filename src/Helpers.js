...
export function getWeekArray(
  ...
  //adding extra weeks
  if (showExtraWeeksNext) {
    //next
    for (let ew = 0; ew < showExtraWeeksNext; ew++)
    {
      const lastWeek = weekArray[weekArray.length - 1];
      ...
      for (let i = 0; i < 7; i++) {
        const day = clone(lastDay);
        day.setDate(lastDay.getDate() + 1 + i);
        extraLastWeek.push(day);
      }
      weekArray.push(extraLastWeek);
    }
  }

  if (showExtraWeeksPrev) {
    //prev
    for (let ew = 0; ew < showExtraWeeksPrev; ew++)
    {
      const firstWeek = weekArray[0];
      ...
      for (let i = 0; i < 7; i++) {
        const day = clone(firstDay);
        day.setDate(firstDay.getDate() - 7 + i);
        extraLastWeek.push(day);
      }
      weekArray.unshift(extraLastWeek);
    }
  }
